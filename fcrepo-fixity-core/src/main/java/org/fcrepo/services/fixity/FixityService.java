...
/**
 * A fixity service implementation This is a standalone service that checks the 
 * fixity values of a fedora datastream. 
 * 
 * <p>It is meant to be run via an {@link ExecutorService}:
 * 
 * <pre>
...
public class FixityService implements Callable<Integer> {
    ...
    /**
    * The loop checking for fixity checks to be done which have been submitted 
    * via {@link #checkObject(String)} or {@link #checkObjects(Collection)}.
    */
    @Override
        ...
        try {
            while (!workQueue.isEmpty()) {
                ...
                logger.debug("gathered " + results.size() +
                        " fixity check results for object '" + id + "'");
    ...
    private List<FixityCheckResult> runChecks(final String pid) {
        final List<FixityCheckResult> results =
                new ArrayList<FixityCheckResult>();
    ...
    /**
    * Request to check an object. The objectId will be queued for work by the 
    * {@link FixityService}
    * 
    * @param objectId
    * @throws IOException
    ...
    public void checkObject(final String... pids) throws IOException {
        for (String pid : pids) {
            if (!this.workQueue.offer(pid)) {
                /* queue is full */
                throw new IOException("Unable to queue check for object " +
                        pid + ". Most likely the queue is full");
            } else {
